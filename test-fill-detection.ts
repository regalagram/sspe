...
import { findSubPathAtPointAdvanced, parsePathString } from './src/utils/path-utils';
...
const result1 = findSubPathAtPointAdvanced(rectPath, pointInside, {
  tolerance: 20,
  includeFill: true,
  includeStroke: true
});
...
const result2 = findSubPathAtPointAdvanced(rectPath, pointNearEdge, {
  tolerance: 20,
  includeFill: true,
  includeStroke: true
});
...
const result3 = findSubPathAtPointAdvanced(rectPath, pointFar, {
  tolerance: 20,
  includeFill: true,
  includeStroke: true
});
...
const linePath = parsePathString('M 50 50 L 150 50');

// Point "inside" the line area (but line is open)
const pointOnLine = { x: 100, y: 50 };
const result4 = findSubPathAtPointAdvanced(linePath, pointOnLine, {
  tolerance: 5,
  includeFill: true,
  includeStroke: true
});
...
const result5 = findSubPathAtPointAdvanced(rectPath, pointInside, {
  tolerance: 20,
  includeFill: false,
  includeStroke: true
});
...
// Test 4: Curved path
const curvePath = parsePathString('M 50 100 C 50 50, 150 50, 150 100 C 150 150, 50 150, 50 100 Z');

const pointInCurve = { x: 100, y: 100 };
const result6 = findSubPathAtPointAdvanced(curvePath, pointInCurve, {
  tolerance: 20,
  includeFill: true,
  includeStroke: true
});
