...
export class SimpleUndoRedo {
  ...
  private getCurrentState(): any {
    ...
    // Ensure the final state has proper array integrity
    if (state) {
      this.ensureArrayIntegrity(state);
      
      // Additional safety for selection
      if (state.selection && typeof state.selection === 'object' && !Array.isArray(state.selection)) {
        const keys = Object.keys(state.selection);
        if (keys.every(k => /^\d+$/.test(k))) {
          const arr = [];
          for (const k of keys.sort((a, b) => parseInt(a) - parseInt(b))) {
            arr[parseInt(k, 10)] = state.selection[k];
          }
          state.selection = arr;
        }
      }
  ...
  private applyMicroDiff(state: any, microDiff: any[]): any {
    ...
    for (const operation of microDiff) {
      ...
      try {
        ...
        console.error('Error applying microdiff operation:', operation, error);
      }
    }
    
    // Ensure arrays remain arrays (microdiff can sometimes convert them to objects)
    this.ensureArrayIntegrity(newState);
    
    // Additional safety: ensure selection is always an array
    if (newState.selection && typeof newState.selection === 'object' && !Array.isArray(newState.selection)) {
      const keys = Object.keys(newState.selection);
      if (keys.every(k => /^\d+$/.test(k))) {
        const arr = [];
        for (const k of keys.sort((a, b) => parseInt(a) - parseInt(b))) {
          arr[parseInt(k, 10)] = newState.selection[k];
        }
        newState.selection = arr;
      }
    }
  ...
  /**
   * Ensure array properties remain arrays after microdiff operations
   */
  private ensureArrayIntegrity(obj: any): void {
    ...
    for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
        ...
        // Convert objects with numeric keys back to arrays
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
          const keys = Object.keys(value);
          const isArrayLike = keys.every(k => /^\d+$/.test(k)) && keys.length > 0;
          
          if (isArrayLike) {
            // Convert back to array, handling sparse arrays
            const indices = keys.map(k => parseInt(k, 10));
            const maxIndex = Math.max(...indices);
            const arr = [];
            
            // Fill array with undefined for sparse arrays, then assign values
            ...
            // Remove undefined elements at the end for dense arrays
          ...
          // Also check for common array property names that should always be arrays
          if (key === 'selection' || key === 'points' || key.endsWith('List') || key.endsWith('Array')) {
            // Force these to be arrays even if they don't look array-like
        ...
        this.ensureArrayIntegrity(value);
      }
    }
  }
