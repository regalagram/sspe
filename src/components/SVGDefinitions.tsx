...
import { useAnimationsForElement } from './AnimationRenderer';
...
const LinearGradientElement: React.FC<{ gradient: LinearGradient }> = ({ gradient }) => {
  const animations = useAnimationsForElement(gradient.id);
  ...
  return (
    ...
    >
      {gradient.stops.map(stop => {
        const stopAnimations = useAnimationsForElement(stop.id);
        return (
          <stop
            key={stop.id}
            offset={`${stop.offset}%`}
...
const PatternElement: React.FC<{ pattern: Pattern }> = ({ pattern }) => {
  const animations = useAnimationsForElement(pattern.id);
